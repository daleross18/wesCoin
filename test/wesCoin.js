...
contract('wesCoin', function(accounts) {
   var tokenInstance;
   ...
   it('initializes the contract with the correct values', function(){
      return wesCoin.deployed().then(function(instance) {
         ...
         assert.equal(name, 'wesCoin', 'has the correct name');
         return tokenInstance.symbol();
      }).then(function(symbol) {
         assert.equal(symbol, 'WES', 'has the correct symbol');
         return tokenInstance.standard();
      }).then(function(standard) {
         assert.equal(standard, 'wesCoin v1.0', 'has the correct standard');
      });
   })
   ...
   it('approves tokens for delegated transfer', function (){
      return wesCoin.deployed().then(function(instance){
         ...
         assert.equal(allowance.toNumber(), 100, 'stores the allowance for delagted transfer');
      });
   });
   ...
   it('handles delegated token transfers', function() {
      return wesCoin.deployed().then(function(instance){
         ...
         assert.equal(receipt.logs.length, 1, 'triggers one event');
         assert.equal(receipt.logs[0].event, 'Transfer', 'should be the "Transfer" event');
         assert.equal(receipt.logs[0].args._from, fromAccount, 'logs the account the tokens are authorized from');
